<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="关系" prop="guanxi">
          <el-select :disabled="ro.guanxi" v-model="ruleForm.guanxi" placeholder="请选择关系">
            <el-option
...
export default {
  data() {
	...
	banji : false,
      ...
      rules: {
          jiazhangzhanghao: [
                { required: true, message: '家长账号不能为空', trigger: 'blur' },
          ],
          mima: [
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.guanxiOptions = "母女,母子,父女,父子,其他".split(',')
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiazhang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiazhangCrossAddOrUpdateFlag = false;
						 this.parent.search();
