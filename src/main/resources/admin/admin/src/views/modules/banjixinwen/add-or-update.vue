...
export default {
  ...
  methods: {
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springboot7z6vr/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `banjixinwen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.banjixinwenCrossAddOrUpdateFlag = false;
						 this.parent.search();
