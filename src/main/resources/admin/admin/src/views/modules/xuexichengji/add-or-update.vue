<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="课程科目" prop="kechengkemu">
          <el-input v-model="ruleForm.kechengkemu" 
              placeholder="课程科目" clearable  :readonly="ro.kechengkemu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="课程科目" prop="kechengkemu">
              <el-input v-model="ruleForm.kechengkemu" 
                placeholder="课程科目" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="分数" prop="fenshu">
          <el-input v-model="ruleForm.fenshu" 
              placeholder="分数" clearable  :readonly="ro.fenshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="分数" prop="fenshu">
              <el-input v-model="ruleForm.fenshu" 
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="考试时间" prop="kaoshishijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.kaoshishijian" 
                type="date"
                :readonly="ro.kaoshishijian"
                placeholder="考试时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.kaoshishijian" label="考试时间" prop="kaoshishijian">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../springboot7z6vr/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuexichengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuexichengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
